/** */
package com.snail.framework.codemaker.service;
...
public class CodeMakerService  {
	 ...
	 * @throws Exception
	 */
	public void generateCode(CodeMakerCfg cfg , CodeMakerJDBCDao codeMakerJDBCDao) throws Exception {
		/** 取得列信息，设置类名及java数据类型 */
		ColumnInfo queryParam = new ColumnInfo();
		queryParam.setDb(cfg.getDb());
		queryParam.setTableName(cfg.getTableName());

		List<ColumnInfo> columnList = codeMakerJDBCDao.selectList(cfg.getDb() , cfg.getTableName());
        ...
        if(!StringUtils.isEmpty(cfg.getTemplateNativePath())) {
            templateBasePath = cfg.getTemplateNativePath();
        } else {
            templateBasePath = CodeMakerCfg.class.getClass().getResource(cfg.getTemplateBasePath()).getFile();
        }
	...
	private boolean makerCode(File templateBaseDir, Set<String> templateNames, CodeMakerCfg cfg, File projectDir,
		...
		// 初始化模板
		Configuration markerCfg = new Configuration(Configuration.VERSION_2_3_22);
		...
		for (String tpName : templateNames) {
			...
			if (classFile.exists()) {
				log.info("文件已存在，跳过生成：{}", classFile.getPath());
				continue;
			} else {
	...
	private File getDataMap(Map<String, Object> dataMap, CodeMakerCfg cfg, List<ColumnInfo> columnList) {
		...
		File subProjectDir = new File(cfg.getTargetPath());
		...
		dataMap.put("baseRepositoryPkg", cfg.getBaseRepositoryPkg());
		dataMap.put("frameworkBasePkg" , cfg.getFrameworkBasePkg());

		// 项目完整包名
		String fullPkg = cfg.getPkgNameType();
		if (!StringUtils.isEmpty(cfg.getPkgNameCompany())) {
			fullPkg += "." + cfg.getPkgNameCompany();
		}
		if (!StringUtils.isEmpty(cfg.getPkgNameProject())) {
			fullPkg += "." + cfg.getPkgNameProject();
		}
		dataMap.put("fullPkgProject", fullPkg);

		// 模块完整包名
		if (!StringUtils.isEmpty(cfg.getPkgNameSubProj())) {
			fullPkg += "." + cfg.getPkgNameSubProj();
		}
		dataMap.put("fullPkgSubProj", fullPkg);
		if (!StringUtils.isEmpty(cfg.getPkgNameModel())) {
